...
//1.加载数据库操作类
	...
	if(!empty($_POST['sub'])){
		 ...
		 if(!empty($title)&&!empty($text)&&!empty($author)){
		 	$rows=$mysql_DB->insert2("schq", array("title"=>$title,"author"=>$author,"text"=>$text,"date"=>date("Y-m-d H:i:s")));
		 	if($rows>0){
		 		echo "<script>
			alert('添加成功！');
 	 		window.location.href='../zph2/files/schq2.php';
 	 		</script>";
		 	}else {
		 			echo "<script>
			alert('添加失败！');
 	 		window.location.href='addschq.php';
 	 		</script>";
		 	}
		 }else{
		 	echo "<script>
...
<div class="MainDiv">
   <form action="addschq.php" method="post" >
<table width="99%" border="0" cellpadding="0" cellspacing="0" class="CContent">

  <tr>
      <th class="tablestyle_title" >市场行情添加页面</th>
  </tr>
		...
		<tr>
			<td width="100%">
				...
				<legend>添加行情</legend>
					  ...
					  <tr>
					    <td  align="right" width="13%">行情标题:</td>

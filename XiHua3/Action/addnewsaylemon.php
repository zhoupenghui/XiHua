...
//1.加载数据库操作类
	...
	if(!empty($_POST['sub'])){
		 ...
		 if(!empty($title)&&!empty($text)&&!empty($author)){
		 	$rows=$mysql_DB->insert2("newsaylemon", array("title"=>$title,"author"=>$author,"text"=>$text,"date"=>date("Y-m-d H:i:s")));
		 	if($rows>0){
		 		echo "<script>
			alert('添加成功！');
 	 		window.location.href='../zph2/files/newsaylemon2.php';
 	 		</script>";
		 	}else {
		 			echo "<script>
			alert('添加失败！');
 	 		window.location.href='addnewsaylemon.php';
 	 		</script>";
		 	}
		 }else{
		 	echo "<script>
...
<div class="MainDiv">
   <form action="addnewsaylemon.php" method="post" >
		...
		<tr>
			<td width="100%">
				...
				<legend>添加新闻</legend>
					  ...
					  <tr>
					    <td  align="right" width="13%">新闻标题:</td>
